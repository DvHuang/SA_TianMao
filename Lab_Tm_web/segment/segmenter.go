...
func DavysegmentsToFeaht(segs []Segment,Featurehashtable map[string]int,dimension int) ( map[string]int,int) {



	for _, seg := range segs { //每一次进去切片中的一个元素，一个切片元仅为一个资源

		if len(seg.token.text)<2 {
			continue
		}


		wordstring := davytextSliceToString(seg.token.text) //返回一个字元

		if _,ok:=Featurehashtable[wordstring];ok {
			Featurehashtable[wordstring] =Featurehashtable[wordstring]
		}else{
			Featurehashtable[wordstring] =dimension
			dimension+=1
		}
	}
	return Featurehashtable,dimension
}


func DavysegmentsTo_Create_Fea_Hash(segs []Segment,feature_Hash map[string]Global_Struct.Fea_str) ( map[string]Global_Struct.Fea_str) {
	...
	for _, seg := range segs { //每一次进去切片中的一个元素，一个切片元仅为一个资源
		...
		if _,ok:=feature_Hash[wordstring];ok {
			linshi.Frequency=feature_Hash[wordstring].Frequency +1
			linshi.Lexical_category=seg.token.pos
			feature_Hash[wordstring]=linshi
		}else{
			linshi.Frequency=1
			linshi.Lexical_category=seg.token.pos
			feature_Hash[wordstring]=linshi
...
func DavysegmentsToString_forfpgrowth(segs []Segment) (map[string]int, string) {
	output := make(map[string]int)
	var stringMapPrefixPath string
	stringMapPrefixPathCount+=1
	for _, seg := range segs { //每一次进去切片中的一个元素，一个切片元仅为一个资源
		...
		if _,ok:=output[wordstring];ok{
			output[wordstring] += 1
		}else{
			output[wordstring] = 1 //一条事务为一个map，map的键为一个项即一个分词,键值需在后面进行更改，这里是多少随便
		}
		stringMapPrefixPath += wordstring

	}
	if output == nil {
		fmt.Print("nothing in the map\n\n\n\n")

	}
	stringMapPrefixPath+=string(stringMapPrefixPathCount)

	return output, stringMapPrefixPath //返回一条事务
}
func DavysegmentsToString_allstr(segs []Segment) (map[string]int, string) {
	output := make(map[string]int)
	var stringMapPrefixPath string
	//stringMapPrefixPathCount+=1 //加这一条是为了防止有两条相同的评论
	for _, seg := range segs { //每一次进去切片中的一个元素，一个切片元仅为一个资源
...
// 将多个字元拼接字符串切片，即事务->项集,进来的是一个词语，而不是一条事务
func davytextSliceToString(text []Text) string {
	...
	return wordstring //返回的是一个字元

}

// 改
/*
func (seg *Segmenter) segmentWords2(text []Text, searchMode bool) map[string]int{
	...
	for current := 0; current < len(text); current++ {
		...
		numTokens := seg.dict.lookupTokens(
			text[current:minInt(current+seg.dict.maxTokenLength, len(text))], tokens)
		//fmt.Printf("lookuptokens text length is %v\n",len(text[current:minInt(current+seg.dict.maxTokenLength, len(text))]))
		//fmt.Printf("lookuptokens text is %v\n",text[current:minInt(current+seg.dict.maxTokenLength, len(text))])
	...
	// 计算各个分词的字节位置
	bytePosition := 0
	//fmt.Printf("len(outputSegments)%v\n\n\n\n",len(outputSegments))
	...
	for iSeg := 0; iSeg < len(outputSegments); iSeg++ {
		...
		twotoone:=TTokenToString(outputSegments[iSeg].token)
		wordMap[twotoone]++
	}
